Shader "Filters/GaussianBlur"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				float4 col = 0;
				col += tex2D(_MainTex, i.kernel0.xy) * 0.0162162162;
				col += tex2D(_MainTex, i.kernel0.zw) * 0.0540540541;
				col += tex2D(_MainTex, i.kernel1.xy) * 0.1216216216;
				col += tex2D(_MainTex, i.kernel1.zw) * 0.1945945946;
				col += tex2D(_MainTex, i.kernel2.xy) * 0.2270270270;
				col += tex2D(_MainTex, i.kernel3.xy) * 0.1945945946;
				col += tex2D(_MainTex, i.kernel3.zw) * 0.1216216216;
				col += tex2D(_MainTex, i.kernel4.xy) * 0.0540540541;
				col += tex2D(_MainTex, i.kernel4.zw) * 0.0162162162;
				return col;
			}
			ENDCG
		}
