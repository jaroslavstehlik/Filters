	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				// Horizontal
				col.rgb += max(tex2D(_MainTex, i.kernel0.xy), tex2D(_MainTex, i.kernel0.zw));
				// Vertical
				col.rgb += max(tex2D(_MainTex, i.kernel1.xy), tex2D(_MainTex, i.kernel1.zw));
				// Diagonal
				col.rgb += max(tex2D(_MainTex, i.kernel2.xy), tex2D(_MainTex, i.kernel2.zw));
				col.rgb += max(tex2D(_MainTex, i.kernel3.xy), tex2D(_MainTex, i.kernel3.zw));
